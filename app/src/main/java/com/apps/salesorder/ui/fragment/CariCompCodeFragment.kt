...
class CariCompCodeFragment : DialogFragment() {

    private val pref by lazy { Preferences(requireActivity()) }
    private lateinit var dataAdapter: CariCompCodeAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        salesagent = requireArguments().getString("salesagent").toString()
    ...
    private fun setupViewModel() {
        binding.title.setText("Cari Company Code")

        val listItems = arrayListOf<Debtor>()
        listItems.addAll(DebtorDao.getAll())

        Timber.e("[DATA] $salesagent")
        Timber.e("[DATA LIST-ITEM] ${listItems.size}")
        setupAdapter(listItems)

    }

    private fun setupAdapter(listItem: ArrayList<Debtor>) {

        dataAdapter = CariCompCodeAdapter(listItem, object : CariCompCodeAdapter.OnAdapterListener{
            override fun onClick(result: Debtor) {
                (activity as SoHeaderActivity?)?.setDataFromDialogCariCompCode(result)
                keluar()
            }
        })
